...
(async () => {
    try {
        const keyWallet = wallets[0];
        const sender = wallets.slice(1); // Exclude the first wallet (sender)

        const amount =  process.env.AMOUNT_TO_SEND
        const amountToSend = ethers.utils.parseEther(amount);

        const gasLimit = await getGasLimit(keyWallet.address,provider);
        ...
        for (let i = 0; i < sender.length; i++) {
            ...
            try {
              ...
              console.log(`Sent ${ethers.utils.formatEther(amountToSend)} ETH from ${senderWallet.address} to ${keyWallet.address} with nonce ${nonce}`);
              } catch (e) {
